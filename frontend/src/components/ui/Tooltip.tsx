...
export default function Tooltip({ content, position="top", children }: TooltipProps) {
  ...
  return (<div className="relative inline-flex" onMouseEnter={()=>set(true)} onMouseLeave={()=>set(false)}>{children}<AnimatePresence>{s&&<motion.div initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}} exit={{opacity:0,scale:0.95}} transition={{duration:0.15}} className={`absolute z-50 ${ps[position]}`}><div className="bg-ink-800 text-ink-50 text-xs px-3 py-1.5 rounded-lg whitespace-nowrap font-medium">{content}<div className={`absolute w-0 h-0 border-4 border-transparent ${ar[position]}`}/></div></motion.div>}</AnimatePresence></div>);
}
